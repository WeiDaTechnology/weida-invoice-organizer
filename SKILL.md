...
description: 整理报销或发票文件夹中的 PDF 发票，提取日期、金额、发票号、销售方和摘要，复制到新文件夹并重命名，同时导出 Excel 报销清单。Use when the user asks to organize reimbursement files, batch rename invoice PDFs, export reimbursement spreadsheets, or process Chinese VAT electronic invoices.
---
...
4. **Excel 导出** - 优先使用模板；模板缺失时自动生成带统计行的报销清单
...
可选模板：`templates/费用报销清单明细表-demo.xlsx`

## 脚本说明

### organize_invoices.py

主处理脚本，串联扫描、提取、重命名和 Excel 导出。
...
### 问题：Excel 导出结果和预期模板不一致

**原因：** 当前目录没有模板，脚本回退到了自动生成表头模式

**解决：** 如需固定格式，请补充 `templates/费用报销清单明细表-demo.xlsx`；如果只需要可用的报销清单，默认导出结果即可直接使用。
...
3. **责任人字段** - 默认留空，可自行填写
4. **字符限制** - 摘要字段会清理 Windows 非法字符，并截断到 20 字符
